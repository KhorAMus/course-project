...
document.getElementById('send_json').onclick = function(){
  sc.send(JSON.stringify({"cmd":["qqqqq"]}));
}
...
lobby_form.addEventListener('close', function() {
  ...
  sc.send(JSON.stringify({"cmd":["disconnected_from_lobby"], "id":lobby_id}));
...
function set_lobby_form_value(lobby_form, lobby_name, lobby_status){
  set_lobby_form_name(lobby_form, lobby_name);
  set_lobby_form_status(lobby_form, lobby_status);
}

function set_lobby_form_name(lobby_form, lobby_name){
...
function set_lobby_form_status(lobby_form, lobby_status){
...
function create_lobby (id){
  ...
  var go_game_lobby_form = document.createElement("button");
      go_game_lobby_form.setAttribute('id', 'go_game_lobby_form');
  ...
  var lobby_form_client_slot = document.createElement("li");
      ...
      go_game_lobby_form.appendChild(document.createTextNode('Go Game'));
      lobby_form_bottom_place.appendChild(close_lobby_form);
      lobby_form_bottom_place.appendChild(go_game_lobby_form);
/////////////////////////////////////////////////////////////////////////////////////////////////Закрыть форму лобби
      ...
      lobby_form_team1_list.onclick = function() { 
        ...
        //sc.send('in_team ' + '1');
        sc.send(JSON.stringify({"cmd":["in_team"], "team":"1"}));
      };
      lobby_form_team2_list.onclick = function() { 
        console.log('sended on server "in_team '+ '2');
        //sc.send('in_team ' + '2');
        sc.send(JSON.stringify({"cmd":["in_team"], "team":"2"}));
      };
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////s
...
document.getElementById('submit_lb').onclick = function() {
  ...
  set_name_pass(value, pass);
    if (lobby_name != ''){
      ...
      sc.send(JSON.stringify({"cmd":["create_lobby", "join_in_lobby"], "name":lobby_name, "pass":lobby_pass, "list":"lobby_form_clients_list"}));
...
sc.socket.onmessage = function(event) {
  //console.log("Получены данные с сервера: " + event.data + "    ");
    ...
    if (cmd[0].command == "connected_to_lobby")
    {
      ...
      set_lobby_form_status(lobby_form, cmd[1].status);
    }
    ...
    if (cmd[0].command == "update_lobby_item")
    {
      update_lobby_item(cmd);
    }

    function update_lobby_form(lobby_form, cmd){
    ...
    function update_lobby_item(cmd){
      var current_lobby = document.getElementById(cmd[1].owner.connection);
      var name1 = 'Empty Slot';
      var name2 = 'Empty Slot';
...
sc.socket.onopen = function() {
  ...
  this.send(JSON.stringify({"cmd":["my_name"], "name":document.getElementById('login').innerHTML}));
...
var create_lobby_item = function(id){
  ...
  var map_preview = document.createElement("img");
      ...
      map_preview.setAttribute('src', 'static/Textures/no_image/map_preview_no_image.png');
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////Присоединиться
      join_form.onclick = function(){
        ...
        sc.send(JSON.stringify({"cmd":["connected_to_lobby"], "id":"lobby_form"+id}));
